    ...
    container.innerHTML = `
      ...
      <div class="card" style="margin-bottom:20px; border-inline-start:4px solid var(--brass);">
        <label class="field-label" style="font-size:12px; font-weight:700; color:var(--ink-soft); display:block; margin-bottom:8px;">
          ${isAr ? 'صف الدورة المستندية المطلوبة باللغة الطبيعية ليقوم الـ AI بتوليد الخريطة:' : 'Describe Business Process in Natural Language:'}
        </label>
        <div style="display:flex; gap:10px;">
          <input type="text" id="flow-prompt-input" class="field" placeholder="${isAr ? 'مثال: دورة شراء الأصول الثابتة، دورة تصنيع وأمر إنتاج، دورة المبيعات والتصدير...' : 'e.g. Fixed asset purchase cycle, production order flow, sales export cycle...'}" style="margin:0; flex:1;">
      ...
      <div id="flow-diagram-container">
        ${renderFlowDiagram(DEFAULT_FLOW)}
      </div>
    `;
    ...
    if (aiBtn && promptInput && diagramBox) {
      aiBtn.addEventListener('click', async () => {
        ...
        if (res.success) {
          ...
          diagramBox.innerHTML = renderFlowDiagram(DEFAULT_FLOW);
        }
      });
    }
  }
