...
const Favorites = (function () {
  ...
  async function render(container) {
    ...
    _favoritesCache.forEach(f => { if (f.type) typesSet.add(f.type); });
      ...
      <div class="toolbar" style="margin-bottom:20px;">
        ...
        <div class="field">
          <select id="fav-mod-filter">
            <option value="">${I18n.t('module.allCategories') !== 'All Categories' ? 'جميع الموديولات' : 'All Modules'}</option>
    ...
    const drawList = () => {
      ...
      listContainer.innerHTML = filtered.map(fav => {
        ...
        const modName = modObj ? I18n.localizedName(modObj) : fav.module_id;
          ...
          <div class="ai-insight-card" data-id="${fav.id}">
            <div class="ai-insight-head">
              <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                <span class="badge" style="background:var(--gold-soft); color:var(--ink); font-weight:600; font-size:11px;">📦 ${Topics.escapeHtml(modName)}</span>
      ...
      listContainer.querySelectorAll('[data-action="remove-fav"]').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          const idx = _favoritesCache.findIndex(f => String(f.id) === String(favId) || String(f.insight_id) === String(insightId));
          let removedObj = null;
          if (idx !== -1) {
            removedObj = _favoritesCache.splice(idx, 1)[0];
          }
          drawList();
          ...
          try {
            ...
            if (removedObj) _favoritesCache.push(removedObj);
            drawList();
            UI.toastError(err);
          }
        });
    ...
    modSelect.addEventListener('change', drawList);
    typeSelect.addEventListener('change', drawList);
  }
